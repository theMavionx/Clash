...
prepare_shared_runtime() {
    ...
    if [ ! -f "$ENV_FILE" ]; then
        if [ -f "$DEPLOY_ROOT/.env" ]; then
            ...
            generated_wallet_enc="$(openssl rand -hex 32)"
            {
                ...
                printf '%s\n' ONDO_PERPS_BUILDER_CODE=clashofperps
    ...
    ensure_env_default "ONDO_PERPS_BUILDER_CODE" "clashofperps"
    ...
    set_env_value "ONDO_PERPS_BUILDER_CODE" "clashofperps"
