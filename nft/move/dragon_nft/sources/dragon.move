// Clash Dragon NFT module on Aptos (Move) — V3-feature-parity mirror
...
module clash_dragon::dragon {
    ...
    const RESOURCE_SEED: vector<u8> = b"clash_dragon::collection";

    // ─── Resources ─────────────────────────────────────────────────

    /// Shared module config — stored under the publisher address (@clash_dragon).
    ...
    /// Publish + initialize. Call once from the admin account (@clash_dragon).
    public entry fun initialize(
        ...
        assert!(admin_addr == @clash_dragon, error::permission_denied(E_NOT_ADMIN));
    ...
    public entry fun set_sale_active(admin: &signer, active: bool) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_dragon).sale_active = active;
    }

    public entry fun set_mint_price(admin: &signer, usd_price_e6: u64) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_dragon).usd_price_e6 = usd_price_e6;
    }

    public entry fun set_upgrade_price(admin: &signer, usd_price_e6: u64) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_dragon).upgrade_usd_price_e6 = usd_price_e6;
    }

    public entry fun set_treasury(admin: &signer, treasury: address) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_dragon).treasury = treasury;
    }
    ...
    public entry fun set_quote_signer(admin: &signer, pubkey: vector<u8>) acquires Config {
        ...
        borrow_global_mut<Config>(@clash_dragon).quote_signer_pubkey = pubkey;
    }

    public entry fun set_max_supply(admin: &signer, new_max: u64) acquires Config {
        assert_admin(admin);
        let cfg = borrow_global_mut<Config>(@clash_dragon);
    ...
    public entry fun init_bridge_fee(admin: &signer, fee_octas: u64) {
        assert_admin(admin);
        if (!exists<BridgeFeeConfig>(@clash_dragon)) {
    ...
    public entry fun set_bridge_fee_octas(admin: &signer, fee_octas: u64) acquires BridgeFeeConfig {
        ...
        if (!exists<BridgeFeeConfig>(@clash_dragon)) {
            ...
            borrow_global_mut<BridgeFeeConfig>(@clash_dragon).fee_octas = fee_octas;
        }
    }
    ...
    inline fun assert_admin(admin: &signer) {
        assert!(signer::address_of(admin) == @clash_dragon, error::permission_denied(E_NOT_ADMIN));
    }
    ...
    public entry fun mint_with_quote(
        ...
        while (i < quantity) {
            ...
            let token_ctor = token::create(
                resource_signer,
                cfg.collection_name,
                string::utf8(b"Dragon NFT"),
                token_name,
                option::none(),
                token_uri,
            );

            // Initialize the token's PropertyMap with unrevealed rarity, then store
    ...
    public entry fun bridge_burn(
        ...
        let stats = borrow_global_mut<BurnStats>(@clash_dragon);
        stats.total_burned = stats.total_burned + 1;
    }

    /// Persistent counter for tokens removed via bridge_burn. Stored at
    /// @clash_dragon (admin) account; created once via `init_burn_stats`.
    ...
    public entry fun init_burn_stats(admin: &signer) {
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == @clash_dragon, error::permission_denied(E_NOT_OWNER));
        if (!exists<BurnStats>(@clash_dragon)) {
        ...
        acquires BurnStats
    {
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == @clash_dragon, error::permission_denied(E_NOT_OWNER));
        assert!(exists<BurnStats>(@clash_dragon), error::invalid_state(E_NOT_OWNER));
        let stats = borrow_global_mut<BurnStats>(@clash_dragon);
    ...
    public fun current_supply(): u64 acquires Config, BurnStats {
        let cfg = borrow_global<Config>(@clash_dragon);
        let burned = if (exists<BurnStats>(@clash_dragon)) {
            borrow_global<BurnStats>(@clash_dragon).total_burned
        } else { 0 };
    ...
    public entry fun bridge_mint(
        ...
        let token_ctor = token::create(
            resource_signer,
            cfg.collection_name,
            string::utf8(b"Dragon NFT (bridged)"),
            token_name,
            option::none(),
            token_uri,
        );

        // Initialize PropertyMap with unrevealed rarity. Off-chain relay
        // immediately syncs the preserved rarity via admin_set_rarity.
        let property_ctor = property_map::prepare_input(
            vector::empty<String>(), vector::empty<String>(), vector::empty<vector<u8>>(),
        );
        property_map::init(&token_ctor, property_ctor);
    ...
    fun bridge_fee_octas_internal(): u64 acquires BridgeFeeConfig {
        if (exists<BridgeFeeConfig>(@clash_dragon)) {
            borrow_global<BridgeFeeConfig>(@clash_dragon).fee_octas
        } else { 0 }
    }
    ...
    public fun get_total_minted(): u64 acquires Config {
        borrow_global<Config>(@clash_dragon).total_minted
    }

    #[view]
    public fun get_max_supply(): u64 acquires Config {
        borrow_global<Config>(@clash_dragon).max_supply
    }

    #[view]
    public fun get_sale_active(): bool acquires Config {
        borrow_global<Config>(@clash_dragon).sale_active
    }

    #[view]
    public fun get_treasury(): address acquires Config {
        borrow_global<Config>(@clash_dragon).treasury
    }

    #[view]
    public fun get_collection_addr(): address acquires Config {
        borrow_global<Config>(@clash_dragon).collection_addr
    }

    #[view]
    public fun get_resource_addr(): address acquires Config {
        borrow_global<Config>(@clash_dragon).resource_addr
    }

    #[view]
    public fun get_quote_signer_pubkey(): vector<u8> acquires Config {
        borrow_global<Config>(@clash_dragon).quote_signer_pubkey
    }

    #[view]
    public fun get_upgrade_price(): u64 acquires Config {
        borrow_global<Config>(@clash_dragon).upgrade_usd_price_e6
    }
