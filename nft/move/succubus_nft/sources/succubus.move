// Clash Succubus NFT module on Aptos (Move) — V3-feature-parity mirror
...
module clash_succubus::succubus {
    use std::error;
    use std::option;
    use std::signer;
    ...
    const RESOURCE_SEED: vector<u8> = b"clash_succubus::collection";

    // ─── Resources ─────────────────────────────────────────────────

    /// Shared module config — stored under the publisher address (@clash_succubus).
    ...
    /// Publish + initialize. Call once from the admin account (@clash_succubus).
    public entry fun initialize(
        ...
        assert!(admin_addr == @clash_succubus, error::permission_denied(E_NOT_ADMIN));
    ...
    public entry fun set_sale_active(admin: &signer, active: bool) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_succubus).sale_active = active;
    }

    public entry fun set_mint_price(admin: &signer, usd_price_e6: u64) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_succubus).usd_price_e6 = usd_price_e6;
    }

    public entry fun set_upgrade_price(admin: &signer, usd_price_e6: u64) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_succubus).upgrade_usd_price_e6 = usd_price_e6;
    }

    public entry fun set_treasury(admin: &signer, treasury: address) acquires Config {
        assert_admin(admin);
        borrow_global_mut<Config>(@clash_succubus).treasury = treasury;
    }
    ...
    public entry fun set_quote_signer(admin: &signer, pubkey: vector<u8>) acquires Config {
        ...
        borrow_global_mut<Config>(@clash_succubus).quote_signer_pubkey = pubkey;
    }

    public entry fun set_max_supply(admin: &signer, new_max: u64) acquires Config {
        assert_admin(admin);
        let cfg = borrow_global_mut<Config>(@clash_succubus);
    ...
    public entry fun init_bridge_fee(admin: &signer, fee_octas: u64) {
        assert_admin(admin);
        if (!exists<BridgeFeeConfig>(@clash_succubus)) {
    ...
    public entry fun set_bridge_fee_octas(admin: &signer, fee_octas: u64) acquires BridgeFeeConfig {
        ...
        if (!exists<BridgeFeeConfig>(@clash_succubus)) {
            ...
            borrow_global_mut<BridgeFeeConfig>(@clash_succubus).fee_octas = fee_octas;
        }
    }

    inline fun assert_admin(admin: &signer) {
        assert!(signer::address_of(admin) == @clash_succubus, error::permission_denied(E_NOT_ADMIN));
    }
    ...
    public entry fun bridge_burn(
        ...
        let stats = borrow_global_mut<BurnStats>(@clash_succubus);
        stats.total_burned = stats.total_burned + 1;
    }

    /// Persistent counter for tokens removed via bridge_burn. Stored at
    /// @clash_succubus (admin) account; created once via `init_burn_stats`.
    ...
    public entry fun init_burn_stats(admin: &signer) {
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == @clash_succubus, error::permission_denied(E_NOT_OWNER));
        if (!exists<BurnStats>(@clash_succubus)) {
        ...
        acquires BurnStats
    {
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == @clash_succubus, error::permission_denied(E_NOT_OWNER));
        assert!(exists<BurnStats>(@clash_succubus), error::invalid_state(E_NOT_OWNER));
        let stats = borrow_global_mut<BurnStats>(@clash_succubus);
    ...
    public fun current_supply(): u64 acquires Config, BurnStats {
        let cfg = borrow_global<Config>(@clash_succubus);
        let burned = if (exists<BurnStats>(@clash_succubus)) {
            borrow_global<BurnStats>(@clash_succubus).total_burned
        } else { 0 };
    ...
    public entry fun bridge_mint(
        ...
        let token_ctor = token::create(
            resource_signer,
            cfg.collection_name,
            string::utf8(b"Succubus NFT (bridged)"),
            token_name,
            option::none(),
            token_uri,
        );
    ...
    fun bridge_fee_octas_internal(): u64 acquires BridgeFeeConfig {
        if (exists<BridgeFeeConfig>(@clash_succubus)) {
            borrow_global<BridgeFeeConfig>(@clash_succubus).fee_octas
        } else { 0 }
    }
    ...
    public fun get_total_minted(): u64 acquires Config {
        borrow_global<Config>(@clash_succubus).total_minted
    }

    #[view]
    public fun get_max_supply(): u64 acquires Config {
        borrow_global<Config>(@clash_succubus).max_supply
    }

    #[view]
    public fun get_sale_active(): bool acquires Config {
        borrow_global<Config>(@clash_succubus).sale_active
    }

    #[view]
    public fun get_treasury(): address acquires Config {
        borrow_global<Config>(@clash_succubus).treasury
    }

    #[view]
    public fun get_collection_addr(): address acquires Config {
        borrow_global<Config>(@clash_succubus).collection_addr
    }

    #[view]
    public fun get_resource_addr(): address acquires Config {
        borrow_global<Config>(@clash_succubus).resource_addr
    }

    #[view]
    public fun get_quote_signer_pubkey(): vector<u8> acquires Config {
        borrow_global<Config>(@clash_succubus).quote_signer_pubkey
    }

    #[view]
    public fun get_upgrade_price(): u64 acquires Config {
        borrow_global<Config>(@clash_succubus).upgrade_usd_price_e6
    }
