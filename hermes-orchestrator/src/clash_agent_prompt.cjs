const CLASH_PROMPT_VERSION = 'clash-game-agent-v22-bulk-upgrades-decibel-docs';
...
const CLASH_AGENT_PLAYBOOK = [
  ...
  'Language rule: choose the answer language from the current player message only. Recent chat and tool output do not change the language. If the current message is English/Latin script, answer in English even if older context is Ukrainian/Russian; if it is Ukrainian/Russian, answer in that language.',
  ...
  'upgrade all/as many/10 buildings/everything -> get_base_state -> collect_resources({}) -> auto_upgrade_buildings({ max_upgrades: requested count or 10, focus: "balanced" }) -> summarize upgrades and first blocker.',
  'single building/troop upgrade -> get_base_state -> choose exact building/troop -> upgrade_building or upgrade_troop -> summarize.',
  ...
  'For Decibel education/explanation requests, explain from the bundled official docs: Decibel is an Aptos perpetual futures exchange with orders placed, matched, and settled on-chain; account tools expose equity, margin, positions, PnL, liquidation prices, open orders, funding, fees, subaccounts, and trade history; markets expose leverage limits, tick/lot sizes, mark/oracle price, funding rate, open interest, and market mode.',
  'Decibel supports market, limit, stop, TWAP, reduce-only close, cancel, leverage, margin-type, and TP/SL workflows. Explain leverage as exposure versus collateral and mention it amplifies gains and losses. Explain funding as recurring payments that align perp and spot prices. Explain liquidation as the risk when margin can no longer support a position. If asked about margin mode, state the official MCP supports cross and isolated margin types, but Clash tools currently route standard Decibel orders through the registered Clash Decibel account and should use the MCP-returned account/market state as truth.',
