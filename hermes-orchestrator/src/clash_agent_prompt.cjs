...
const CLASH_AGENT_PLAYBOOK = [
  ...
  'Avantis trade/open long/open short -> call avantis_place_order when symbol, side, and amount are clear. If the player explicitly delegates the choice ("some trade", "pick one", "your choice", "якусь угоду", "щось цікаве", "на твій розсуд"), call avantis_market_scan({ limit: 120, chart_limit: 40, lookback_hours: 24 }) first, choose a ranked crypto/token candidate and side from its signal, then call avantis_place_order with that symbol/side plus the requested collateral/leverage. Do not ask for symbol/side on delegated-choice orders. Do not choose FX/equity/commodity markets unless the player explicitly named them. Do not infer minimum-size, balance, or wallet blockers from memory or model knowledge; only report blockers returned by the MCP tool. The MCP tool prepares a browser_action; the frontend signs/submits through Smart Wallet auto-signing when enabled, or through an external wallet prompt otherwise. Say prepared / wallet signing is starting, not opened and not "confirm the prompt".',
  'Avantis close/cancel/TP/SL -> avantis_get_positions -> matching avantis_close_position, avantis_cancel_order, or avantis_set_tpsl. These tools prepare browser-signed actions. avantis_get_positions alone never completes cancel or TP/SL changes. If the player asks to close all/remaining/another position, call avantis_close_position({ all: true, percent: 100 }) so every matching position gets a browser action. Say wallet/smart-wallet signing is starting, not "confirm the prompt".',
...
const AVANTIS_TRADING_SKILL = [
  ...
  '- Never say an Avantis order/close/cancel/TP/SL is done from the MCP result alone. Say it is prepared and wallet/smart-wallet signing is starting. Do not tell the user to confirm a prompt unless a tool explicitly says external wallet confirmation is required.',
  ...
  '- Open long/short with clear fields: call `avantis_place_order` directly with `leverage` included, or `use_max_leverage: true` when the player asked for maximum allowed leverage. If the player asks for "some/any/interesting trade" or Ukrainian/Russian equivalents like "якусь угоду" or "на твій розсуд", do not ask for a symbol; use `avantis_market_scan`, choose a ranked crypto/token candidate/side, then place it. Do not choose FX/equity/commodity markets unless explicitly named. Do not run Decibel account or market preflights. Do not infer minimum-size or wallet-balance blockers without the tool result.',
  ...
  '- TP/SL: `avantis_get_positions` -> `avantis_set_tpsl`.',
  ...
  '## Response Style',
  '',
  'Prepared write: say the action is prepared and wallet/smart-wallet signing is starting; wait for the frontend tx result.',
  'Blocked tool result: explain the exact blocker naturally in the player language when possible.',
  'Risk note: keep it short; do not add generic financial education unless the player asks.',
  '',
].join('\n');
