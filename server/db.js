...
try {
  db.exec(`
    CREATE TABLE IF NOT EXISTS tournaments (
      ...
      name         TEXT NOT NULL,
      description  TEXT,
      dex          TEXT NOT NULL CHECK(dex IN ('pacifica','avantis','domfi','etoro','decibel','dango','gmx','ostium','monad','phoenix','hyperliquid','risex','nado','ondo','leverup','hibachi','hotstuff','grvt','katana','gmtrade','flash','lighter','rhlighter','bulk','imperial')),
  ...
  try {
    ...
    const needsRebuild = schema
      && (!schema.includes("event_kind") || !schema.includes("'points'") || !schema.includes("'volume_trophies_50_50'") || !schema.includes("'dango'") || !schema.includes("'domfi'") || !schema.includes("'etoro'") || !schema.includes("'ostium'") || !schema.includes("'monad'") || !schema.includes("'phoenix'") || !schema.includes("'hyperliquid'") || !schema.includes("'risex'") || !schema.includes("'nado'") || !schema.includes("'ondo'") || !schema.includes("'leverup'") || !schema.includes("'hibachi'") || !schema.includes("'grvt'") || !schema.includes("'katana'") || !schema.includes("'gmtrade'") || !schema.includes("'flash'") || !schema.includes("'lighter'") || !schema.includes("'rhlighter'") || !schema.includes("'bulk'") || !schema.includes("'imperial'") || !schema.includes("points_trophy_weight") || !schema.includes("scoring_mode") || !schema.includes("daily_pool_points") || !schema.includes("daily_pool_growth_pct") || !schema.includes("daily_pool_overrides") || !schema.includes("daily_pool_award_time_utc") || !schema.includes("prize_tiers") || !schema.includes("mega_config") || !schema.includes("reward_config") || !schema.includes("rewards_in_cop") || !schema.includes("seeker_only") || !schema.includes("seeker_gold_boost") || !schema.includes("shield_hours") || !schema.includes("dex_scope") || !schema.includes("eligible_dexes") || !schema.includes("dex_vs_dex") || !schema.includes("team_prize_splits") || !schema.includes("attack_match_policy"));
    if (needsRebuild) {
      ...
      db.transaction(() => {
        db.exec(`
          ...
          SELECT
            ...
            name, description,
            CASE WHEN dex IN ('pacifica','avantis','domfi','etoro','decibel','dango','gmx','ostium','monad','phoenix','hyperliquid','risex','nado','ondo','leverup','hibachi','hotstuff','grvt','katana','gmtrade','flash','lighter','rhlighter','bulk','imperial') THEN dex ELSE 'pacifica' END,
            ...
            CASE
              ...
              ELSE '["' || CASE WHEN dex IN ('pacifica','avantis','domfi','etoro','decibel','dango','gmx','ostium','monad','phoenix','hyperliquid','risex','nado','ondo','leverup','hibachi','hotstuff','grvt','katana','gmtrade','flash','lighter','rhlighter','bulk','imperial') THEN dex ELSE 'pacifica' END || '"]'
            END,
...
const stmts = {
  ...
  getActiveTournamentForPlayer: db.prepare(`
    SELECT t.id AS tournament_id, t.dex, t.dex_scope, t.eligible_dexes, t.mode, t.seeker_only,
           ...
           t.shield_hours, t.start_at, t.end_at, p.joined_at, p.left_at
...
function getBuildingMaxLevelForTownHall(type, townHallLevel) {
  const defMax = Math.max(1, Number(BUILDING_DEFS[type]?.max_level) || 1);
  const levels = TH_MAX_LEVEL[type];
  if (!Array.isArray(levels) || levels.length === 0) {
    return Math.min(defMax, Math.max(1, Number(townHallLevel) || 1));
  }
  const index = Math.max(0, Math.min(levels.length - 1, (Number(townHallLevel) || 1) - 1));
...
const TROOP_DEFS = {
  ...
  horror: {
    ...
    cost: [
      ...
      { gold: 1500, wood: 0, ore: 1500 },
      { gold: 3000, wood: 0, ore: 3000 },
      { gold: 5400, wood: 0, ore: 5400 },
      { gold: 9000, wood: 0, ore: 9000 },
    ],
  },
  barbarian: {
    max_level: 7,
...
function normalizeTroopTypeKey(troopType) {
  ...
  if (compact === 'icegolem') return 'ice_golem';
  if (compact === 'horrorevolution') return 'horror';
...
function upgradeTroop(playerId, troopType, options = {}) {
  ...
  const currentTownHallLevel = getTownHallLevel(playerId);
  const requiredTownHallLevel = Math.max(1, Number(def.min_town_hall_level) || 1);
  if (currentTownHallLevel < requiredTownHallLevel) {
    return {
      error: `Upgrade Town Hall to level ${requiredTownHallLevel} to unlock this troop`,
...
function getTroopTownHallUnlock(playerId, troopType) {
  const troopKey = normalizeTroopTypeKey(troopType);
  const def = TROOP_DEFS[troopKey];
  if (!def || isTroopDisabled(troopKey)) {
    return {
      unlocked: false,
      troop_type: troopKey,
      error: `Unknown troop type: ${troopType}`,
      code: 'UNKNOWN_TROOP',
      status: 400,
    };
  }
  ...
  const requiredTownHallLevel = Math.max(1, Number(def.min_town_hall_level) || 1);
  return {
    unlocked: currentTownHallLevel >= requiredTownHallLevel,
    troop_type: troopKey,
    current_town_hall_level: currentTownHallLevel,
    required_town_hall_level: requiredTownHallLevel,
    error: currentTownHallLevel >= requiredTownHallLevel
      ? null
      : `Upgrade Town Hall to level ${requiredTownHallLevel} to unlock this troop`,
    code: currentTownHallLevel >= requiredTownHallLevel ? null : 'TOWN_HALL_LEVEL_REQUIRED',
    status: currentTownHallLevel >= requiredTownHallLevel ? 200 : 403,
  };
}
...
function matchTroopEntryLevel(entry, fallback = 1) {
  ...
  const maxLevel = Math.max(1, Number(TROOP_DEFS[troopType]?.max_level) || 1);
  const match = String(entry || '').match(/:L([1-7])(?:$|:)/i);
  if (match) return clampMatchNumber(match[1], 1, maxLevel, fallback);
  return clampMatchNumber(fallback, 1, maxLevel, 1);
}

function troopLevelsObject(playerId) {
  const levels = {};
  for (const row of getTroopLevels(playerId)) {
    const troopType = normalizeMatchTroopType(row.troop_type);
    const maxLevel = Math.max(1, Number(TROOP_DEFS[troopType]?.max_level) || 1);
    levels[troopType] = clampMatchNumber(row.level, 1, maxLevel, 1);
  }
  return levels;
}
...
function troopPowerFromEntry(entry, levelMap) {
  ...
  if (troopType === 'horror') {
    const root = HORROR_EVOLUTION.stages?.[0]?.[level];
    const medium = HORROR_EVOLUTION.stages?.[1]?.[level];
    const small = HORROR_EVOLUTION.stages?.[2]?.[level];
    if (!root || !medium || !small) return 0;
    const familyHp = (
      Number(root.hp)
      + HORROR_EVOLUTION.childrenPerSplit * Number(medium.hp)
      + HORROR_EVOLUTION.childrenPerSplit ** 2 * Number(small.hp)
    );
    const phaseDps = Math.max(
      Number(root.damage) / Math.max(0.1, Number(root.atkSpeed)),
      HORROR_EVOLUTION.childrenPerSplit
        * Number(medium.damage) / Math.max(0.1, Number(medium.atkSpeed)),
      HORROR_EVOLUTION.childrenPerSplit ** 2
        * Number(small.damage) / Math.max(0.1, Number(small.atkSpeed)),
    );
    return familyHp * 0.18 + phaseDps * 28;
  }
...
function defensePowerForBuilding(building) {
  ...
  if (type === 'shark_trap') {
    const sharkDamageLevels = BUILDING_DEFS.shark_trap.damage_levels || [500];
    const damage = sharkDamageLevels[Math.max(0, Math.min(sharkDamageLevels.length - 1, level - 1))] || 500;
    return damage * 1.4;
  }
  return 0;
}
...
function replaySimDebug(simResult) {
  ...
  const debug = {
    ...
    deployedTroopsSpawned: simResult._deployedTroopsSpawned,
