...
const plugin={name:'position-action-verification',enforce:'pre',
 ...
 load(id){if(id!==entry)return;return `
  ...
  const imperial={dex:'imperial',source:'imperial',symbol:'BTC',side:'bid',amount:0.010600000000077681,size_usd:846.0401,entry_price:79815.103773,mark_price:79820,margin:4.135005,unrealized_pnl:-.2661395886,pnl_pct:-6.44,leverage:190.74,is_isolated:true,pair_index:2,trade_index:'fixture:phoenix',pnl_includes_fees:true};
  const bulk=normalizeBulkPosition({symbol:'BTC-USD',size:.000609,price:79795.4,fairPrice:79994.4,unrealizedPnl:.121191,leverage:20,iso:false});
  function App(){const [layout,setLayout]=useState('table'),[venue,setVenue]=useState('BULK'),[alert,setAlert]=useState(''),[calls,setCalls]=useState([]),[enabled,setEnabled]=useState(false),[tp,setTp]=useState(''),[sl,setSl]=useState(''),[mode,setMode]=useState('price'),[side,setSide]=useState('bid');
   const dex=venue==='BULK'?'bulk':'imperial',position=venue==='BULK'?bulk:venue==='Imperial index'?{...imperial,mark_price:79982.02,unrealized_pnl:1.4005,pnl_pct:33.87,live_mark_basis:'index'}:imperial;
   const base={positions:[position],orders:[],prices:[],dataReady:true,leverageSettings:{},marginModes:{},loading:false,error:null,localAlert:alert,setLocalAlert:setAlert,clearError:()=>setAlert(''),dex,markets:[],account:{},
   ...
   return <main><h1>Position actions · local verification</h1><p>No trading requests. Calls: {calls.length}</p><nav>{['BULK','Imperial venue','Imperial index'].map(value=><button key={value} onClick={()=>setVenue(value)}>{value}</button>)}</nav><nav>{['table','cards','order'].map(value=><button key={value} onClick={()=>{setAlert('');setLayout(value);}}>{value}</button>)}</nav>
