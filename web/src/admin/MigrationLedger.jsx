    ...
    {data && <>
      ...
      <div className="table-wrap"><table><thead><tr><th>Request / time</th><th>Verified sender / recipient</th><th>Amounts</th><th>State / confirmation</th><th>Transactions</th></tr></thead>
        <tbody>{data.requests.map(row => <tr key={row.id}>
          ...
          <td style={{ minWidth: 200 }}>{migrationStateText(row.status, row.payoutIncludedAt)}<br/>Deposit: {row.depositedAt ? formatUtc(row.depositedAt) : 'Not confirmed'}{row.payoutNotBefore && <><br/>Payout eligible after: {formatUtc(row.payoutNotBefore)}</>}<br/>Payout: {row.paidAt ? formatUtc(row.paidAt) : 'Not confirmed'}{row.errorCode && <p>{row.errorCode}</p>}</td>
