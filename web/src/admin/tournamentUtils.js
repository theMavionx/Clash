export const TOURNAMENT_DEXES = [
  ...
  'risex', 'nado', 'ondo', 'hibachi', 'grvt', 'hotstuff', 'katana', 'gmtrade', 'flash', 'lighter', 'rhlighter', 'bulk', 'imperial',
];
...
export function tournamentToForm(tournament) {
  ...
  return {
    ...
    ranked_altar_bonus_cap: Math.max(0, Math.min(100, Math.floor(Number(tournament.ranked_altar_bonus_cap || 0) || 0))),
...
export function formToTournamentBody(form) {
  ...
  return {
    ...
    ranked_altar_bonus_cap: Math.max(0, Math.min(100, Math.floor(Number(form.ranked_altar_bonus_cap || 0) || 0))),
