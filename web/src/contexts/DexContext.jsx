...
export const DEX_CONFIG = {
  ...
  rhlighter: {
    ...
    logo: '/lighter.svg',
    logoLabel: 'Lighter',
    companionLogo: '/robinhood.svg',
    companionLogoLabel: 'Robinhood',
    logoIsWordmark: false,
    color: '#16A34A',
    colorDark: '#15803D',
...
export function DexProvider({ children }) {
  ...
  const [dex, setDexState] = useState(
    () => {
      ...
      if (isDexAvailableInContext(cached, ctx)) return cached;
      localStorage.setItem(STORAGE_KEY, 'pacifica');
      if (cached === 'dango') localStorage.removeItem('clash_dex_picked');
      return 'pacifica';
    }
  );
