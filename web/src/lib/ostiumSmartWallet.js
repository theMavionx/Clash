...
import { OSTIUM_PRIMARY_RPC_URL } from './ostiumContract';
...
export function getOrCreateOstiumSmartWalletDelegate(owner) {
  const key = String(owner || '').toLowerCase();
...
export function forgetOstiumSmartWalletDelegate(owner) {
  const key = storageKey(owner);
  removeStoredPayload(key);
}

/** Import delegate key pasted in Bots (same storage as Smart Wallet flow). */
export function importOstiumSmartWalletDelegate(owner, privateKey) {
  const key = String(owner || '').toLowerCase();
