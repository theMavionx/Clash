...
import { OSTIUM_PRIMARY_RPC_URL } from './ostiumContract.js';

const STORAGE_PREFIX = 'clash_ostium_smart_wallet_delegate_v1';
const DEFAULT_TTL_MS = 24 * 60 * 60 * 1000;
const MIN_VALID_MS = 2 * 60 * 1000;

export const OSTIUM_SMART_WALLET_MIN_ETH = 600000000000000n; // 0.0006 ETH
...
function ttlMs() {
  const hours = Number(import.meta.env.VITE_OSTIUM_SMART_WALLET_TTL_HOURS || 24);
...
export function readOstiumSmartWalletDelegate(owner) {
...
export function getOrCreateOstiumSmartWalletDelegate(owner, options = {}) {
  const key = String(owner || '').toLowerCase();
  if (!isAddress(key)) throw new Error('Connect your Arbitrum wallet first');
  const scope = options.scope || captureCredentialScope();
  assertCredentialScope(scope);
  const existing = readOstiumSmartWalletDelegate(key);
...
export function forgetOstiumSmartWalletDelegate(owner, options = {}) {
  const key = storageKey(owner);
  return removeStoredPayload(key, options);
}

/** Import delegate key pasted in Bots (same storage as Smart Wallet flow). */
export function importOstiumSmartWalletDelegate(owner, privateKey, options = {}) {
  const key = String(owner || '').toLowerCase();
  if (!isAddress(key)) throw new Error('Connect your Arbitrum wallet (0x…).');
  const raw = String(privateKey || '').trim();
  const pk = raw.startsWith('0x') ? raw : `0x${raw}`;
  if (!isPrivateKey(pk)) throw new Error('Invalid Ostium delegate private key (0x + 64 hex).');
  const record = fromPrivateKey(pk, Date.now() + ttlMs());
  persist(key, record, options);
  return record;
}

export function createOstiumSmartWalletClient(delegate, rpcUrl = OSTIUM_PRIMARY_RPC_URL) {
  if (!delegate?.privateKey) throw new Error('Ostium Smart Wallet delegate is missing');
  return createWalletClient({
    account: privateKeyToAccount(delegate.privateKey),
    chain: arbitrum,
    transport: http(rpcUrl || OSTIUM_PRIMARY_RPC_URL),
  });
}
