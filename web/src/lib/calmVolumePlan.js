...
export function maxSafeTradeSizeUsd(availableUsd, exchangeId = '') {
  ...
  // Below venue floor → cannot quote dual-leg safely (e.g. Nado $100).
...
export function planCalmFromBalance({
  ...
  const maxPositionUsd = tradeSizeUsd <= 0
  ...
  if (avail == null) {
    ...
    hint = minOrder >= 100
      ...
      ? `${String(exchangeId || 'Venue').replace(/^./, (c) => c.toUpperCase())} floor is $${minOrder}/order. Free ≈$${avail.toFixed(2)} at ${v.leverage}× cannot fund dual-leg quotes — deposit ≥~$${minDeposit} USDC (venue minimum, not an API quota). ${levNote}`

      : `Free margin too low for a safe dual-leg quote. Deposit more USDC. ${levNote}`;
