...
func _render_portrait() -> void:
	...
	if player == null or not player.has_animation("Melee_1H_Attack_Chop"):
		push_error("Pirate knight UI portrait failed: attack animation is unavailable.")
		quit(1)
		return
	player.play("Melee_1H_Attack_Chop")
	player.speed_scale = 0.0
	player.seek(player.current_animation_length * 0.30, true)

	for _frame in 4:
		await process_frame

	var sword := character.find_child("OHS07_Sword_R", true, false) as MeshInstance3D
	if sword == null or not sword.visible:
		push_error("Pirate knight UI portrait failed: sword is missing.")
		quit(1)
		return
	...
	if error != OK:
		push_error("Pirate knight UI portrait failed: %s" % error_string(error))
		quit(1)
		return

	print(
		"[PIRATE_KNIGHT_UI] PASS path=", output_path,
...
func _output_path() -> String:
	var path := ProjectSettings.globalize_path("res://web/src/assets/units/knight.png")
