...
func _render_portrait() -> void:
	...
	if player == null or not player.has_animation("Ranged_Magic_Spellcasting"):
		push_error("Pirate mage UI portrait failed: cast animation is unavailable.")
		quit(1)
		return
	player.play("Ranged_Magic_Spellcasting")
	player.speed_scale = 0.0
	player.seek(player.current_animation_length * 0.20, true)

	for _frame in 4:
		await process_frame

	var wand := character.find_child("Wand03", true, false) as MeshInstance3D
	if wand == null or not wand.visible:
		push_error("Pirate mage UI portrait failed: wand is missing.")
		quit(1)
		return
	...
	if error != OK:
		push_error("Pirate mage UI portrait failed: %s" % error_string(error))
		quit(1)
		return

	print(
		"[PIRATE_MAGE_UI] PASS path=", output_path,
...
func _output_path() -> String:
	var path := ProjectSettings.globalize_path("res://web/src/assets/units/mage.png")
