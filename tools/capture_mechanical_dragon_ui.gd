...
const MECHANICAL_DRAGON_SCRIPT: Script = preload("res://scripts/mechanical_dragon.gd")
...
func _render_portrait() -> void:
	...
	character.set_script(MECHANICAL_DRAGON_SCRIPT)
	...
	if player == null or not player.has_animation("Idle_A"):
		push_error("Mechanical Dragon UI portrait failed: idle animation is unavailable.")
		quit(1)
		return
	player.play("Idle_A")
	...
	if error != OK:
		push_error("Mechanical Dragon UI portrait failed: %s" % error_string(error))
		quit(1)
		return

	var used_rect := image.get_used_rect()
	if used_rect.size.x < 120 or used_rect.size.y < 80:
		push_error("Mechanical Dragon UI portrait failed: rendered model is unexpectedly small.")
		quit(1)
		return
	print(
		"[MECHANICAL_DRAGON_UI] PASS path=", output_path,
...
func _add_lighting(stage: Node3D) -> Camera3D:
	...
	environment.ambient_light_color = Color("#bfe8ff")
	...
	key_light.light_energy = 0.55
	stage.add_child(key_light)

	var fill_light := DirectionalLight3D.new()
	fill_light.rotation_degrees = Vector3(-18, 150, 0)
	fill_light.light_color = Color("#74dfff")
...
func _frame_camera(character: Node3D, camera: Camera3D) -> void:
	...
	for mesh_node in character.find_children("*", "MeshInstance3D", true, false):
		...
		has_bounds = true
	if not has_bounds:
		push_error("Mechanical Dragon UI portrait failed: model has no meshes.")
		quit(1)
		return
	var center := bounds.get_center()
	var view_direction := Vector3(0.46, 0.25, 1.0).normalized()
	var distance := maxf(bounds.size.x, bounds.size.z) * 2.15
...
func _output_path() -> String:
	var path := ProjectSettings.globalize_path(
		"res://web/src/assets/units/mechanical_dragon.png"
	)
