...
$NodeFiles = @(
    ...
    "server/test-leverup-reward-flow.js",
    "server/test-leverup-tournament-migration.js",
    ...
    "server-futures/test-leverup-rewards.js",
...
Invoke-Step "LeverUp proxy transport and submission safety" { node --test server-futures/test-leverup-transport.js }
...
Invoke-Step "LeverUp tournament schema migration" { node server/test-leverup-tournament-migration.js }
...
if ($Mode -in @("Full", "Deploy")) {
    ...
    Invoke-Step "CLASH migration UI amount and state regressions" {
        node --test web/src/migration/model.test.mjs web/src/migration/solana-key-preview.test.mjs web/src/migration/solana-mnemonic.test.mjs web/src/migration/target-asset.test.mjs
    }
