...
const ICE_GOLEM_SCRIPT: Script = preload("res://scripts/ice_golem.gd")
...
func _render_portrait() -> void:
	...
	character.set_script(ICE_GOLEM_SCRIPT)
	character.scale = Vector3.ONE * 0.1
	character.rotation_degrees.y = -28.0
	...
	if player == null or not player.has_animation("Idle_A"):
		push_error("Ice Golem UI portrait failed: idle animation is unavailable.")
		quit(1)
		return
	player.play("Idle_A")
	...
	var output_path := ProjectSettings.globalize_path("res://web/src/assets/units/ice_golem.png")
	DirAccess.make_dir_recursive_absolute(output_path.get_base_dir())
	var error := image.save_png(output_path)
	if error != OK:
		push_error("Ice Golem UI portrait failed: %s" % error_string(error))
		quit(1)
		return

	var used_rect := image.get_used_rect()
	if used_rect.size.x < 120 or used_rect.size.y < 120:
		push_error("Ice Golem UI portrait failed: rendered model is unexpectedly small.")
		quit(1)
		return
	print("[ICE_GOLEM_UI] PASS path=", output_path, " used_rect=", used_rect)
	viewport.free()
	await process_frame
	quit()
...
func _add_lighting(stage: Node3D) -> Camera3D:
	...
	fill_light.light_color = Color("#8bc5df")
...
func _frame_camera(character: Node3D, camera: Camera3D) -> void:
	...
	for mesh_node in character.find_children("*", "MeshInstance3D", true, false):
		...
		has_bounds = true
	if not has_bounds:
		push_error("Ice Golem UI portrait failed: model has no meshes.")
		quit(1)
		return
	var center := bounds.get_center()
	var view_direction := Vector3(0.48, 0.24, 1.0).normalized()
	var distance := maxf(bounds.size.x, bounds.size.z) * 2.2
