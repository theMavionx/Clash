...
func _initialize() -> void:
	...
	_expect(BuildingSystemScript.TH_MAX_COUNT.get("harpoon", []) == [0, 0, 0, 0, 0, 1, 1, 2, 2], "Harpoon count must stay at one through TH7 and rise to two at TH8.")
	_expect(BuildingSystemScript.TH_MAX_LEVEL.get("harpoon", []) == [1, 1, 1, 1, 1, 6, 7, 8, 9], "Harpoon level must track TH6-TH9.")
	_expect(BuildingSystemScript.TH_MAX_LEVEL.get("tombstone", []).slice(0, 7) == [1, 2, 3, 4, 4, 5, 6], "Tombstone must reach L6 at TH7.")
	...
	_expect(harpoon.get("hp_levels", []) == [1800, 2400, 3200, 4300, 5600, 6756, 10201, 12000, 13800], "Harpoon must preserve its survivability-focused nine-level HP curve.")
	_expect(harpoon.get("damage_levels", []) == [45, 55, 65, 75, 77, 82, 98, 100, 112], "Harpoon must define the approved nine-level damage curve.")
	_expect(harpoon.get("range_levels", []) == [1.20, 1.27, 1.45, 1.64, 1.82, 1.95, 2.08, 2.20, 2.30], "Harpoon range must preserve its authored anti-air coverage curve.")
	_expect(harpoon.get("pull_speed_levels", []) == [0.85, 0.92, 0.99, 1.06, 1.13, 1.20, 1.40, 1.48, 1.55], "Harpoon pull speed must grow across all nine levels.")
	...
	_expect(is_equal_approx(harpoon_runtime.detect_range, 2.30), "Harpoon L9 range must cover the authored TH9 engagement radius.")
