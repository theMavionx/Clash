...
func _initialize() -> void:
	...
	_expect(mortar.get("damage_levels", []) == [95, 108, 158, 227, 284, 370, 487], "Mortar must define the approved seven-level damage curve.")
	...
	_expect(mortar_runtime.damage == 487, "Mortar L7 runtime damage must be 487.")
	...
	_expect(harpoon.get("hp_levels", []) == [1800, 2400, 3200, 4300, 5600, 7200, 10000, 12000], "Harpoon must define the approved eight-level HP curve.")
	...
	_expect(cannon.get("damage_levels", []) == [40, 109, 259, 431, 631, 759, 1080], "Cannon must define the approved seven-level damage curve.")
