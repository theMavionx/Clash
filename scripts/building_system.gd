...
func _send_main_ship_panel(ship: Dictionary) -> void:
	...
	bridge.send_to_react("building_selected", {
		...
		"ship_max_same_troop_slot_share_bps": int(ship.get("composition_policy", {}).get("max_same_troop_slot_share_bps", 5000)),
		"ship_same_troop_slot_limit": int(ship.get("composition_policy", {}).get("same_troop_slot_limit", ceili(float(ship.get("capacity", troops.size())) * 0.5))),
		"ship_troop_copy_limits": ship.get("composition_policy", {}).get("troop_copy_limits", {"fire_dragon": 1}),
...
func _apply_main_ship_state_from_server(ship_data: Dictionary) -> void:
	...
	if bridge:
		...
		bridge.send_to_react("ship_updated", {
			...
			"ship_max_same_troop_slot_share_bps": int(ship_data.get("composition_policy", {}).get("max_same_troop_slot_share_bps", 5000)),
			"ship_same_troop_slot_limit": int(ship_data.get("composition_policy", {}).get("same_troop_slot_limit", ceili(float(server_capacity) * 0.5))),
			"ship_troop_copy_limits": ship_data.get("composition_policy", {}).get("troop_copy_limits", {"fire_dragon": 1}),
