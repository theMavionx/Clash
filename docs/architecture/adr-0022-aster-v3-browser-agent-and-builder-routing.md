...
5. Builder configuration is server-owned through `ASTER_BUILDER_ADDRESS` and `ASTER_BUILDER_FEE_RATE`. The fallback fee rate is the official Aster Code demo value `0.00001`; production can override it after the commercial rate is confirmed. New/opening orders fail closed until the address is configured and approved. Risk-reducing close and cancel actions remain available.
